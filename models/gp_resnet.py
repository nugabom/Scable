...
class Model(nn.Module):
    def __init__(self, num_classes=10, input_size=32):
        ...
        self.channels = make_divisible(16 * self.width_mult)
        self.conv1 = USConv2d(3, self.channels, 1, bias=False,
                              us=[False, True])
        self.bn1 = DynamicGroupBatchNorm2d(self.channels)
        self.relu1 = nn.ReLU(inplace=True)

        self.layer1 = self._make_layer(BasicBlock, 16, self.block_setting[0], stride=1)
        self.layer2 = self._make_layer(BasicBlock, 32, self.block_setting[1], stride=2)
        self.layer3 = self._make_layer(BasicBlock, 64, self.block_setting[1], stride=2)

        self.classifier = USLinear(64, num_classes, us=[True, False])
